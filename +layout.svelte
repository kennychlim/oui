<script lang="ts">
	...
	const setupSocket = async (enableWebsocket) => {
		...
		_socket.on('connect', () => {
			console.log('connected line 92:', _socket.id);
		});
	...
	onMount(async () => {
		...
		if (window?.electronAPI) {
			...
			if (info) {
				isApp.set(true);
				appInfo.set(info);
				console.log('info line 492:', info)
		...
		if (browser) {
			...
			const savedIsApp = localStorage.getItem('isApp');
			
			if (savedView === 'dashboard' || savedIsApp === 'true') {
				isApp.set(true);
			}
			console.log('Storage values(line 662):', { browser, savedView, savedIsApp, isApp: $isApp });
		};
	...
	$: if (browser && $page.url.pathname.includes('/dashboard')) {
		isApp.set(true);
		console.log('onMount values(line 672):', { browser, isApp: $isApp });
	}		
</script>
